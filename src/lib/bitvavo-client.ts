...
export class BitvavoClient extends Context.Tag("BitvavoClient")<
  BitvavoClient,
  {
    readonly getBalances: Effect.Effect<BitvavoBalance[], Error>;
    readonly getTrades: (
      market?: string,
    ) => Effect.Effect<BitvavoTrade[], Error>;
    readonly getTicker: (market: string) => Effect.Effect<BitvavoTicker, Error>;
  }
>() {}
...
export const BitvavoClientLive = Layer.effect(
  BitvavoClient,
  Effect.gen(function* () {
    const apiKey = yield* Effect.fromNullable(process.env.BITVAVO_API_KEY).pipe(
      Effect.orElseFail(
        () => new Error("BITVAVO_API_KEY environment variable is required"),
      ),
    );

    const apiSecret = yield* Effect.fromNullable(
      process.env.BITVAVO_API_SECRET,
    ).pipe(
      Effect.orElseFail(
        () => new Error("BITVAVO_API_SECRET environment variable is required"),
      ),
    );
        ...
        for (const tradeMarket of markets) {
          ...
          const response = yield* Effect.tryPromise({
            try: () => client.get<any[]>(endpoint, { headers }),
              ...
              new Error(`Failed to fetch trades for ${tradeMarket}: ${error}`),
          });
